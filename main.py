            ...
            if slot.item is not None:
                ...
                if slot.count > 0:
                    ...
                    text_rect = count_text.get_rect(
                        bottomright=(slot_x + SLOT_SIZE - 5, slot_y + SLOT_SIZE - 5)
                    )
...
class Player(Entity, HasInventory):
    ...
    def draw_health(this):
        ...
        for i in range(full_hearts):
            SURF.blit(
                this.full_heart_texture, (HEART_X_START + i * HEART_SPACING, HEART_Y)
            )
        # Draw half heart
        if half_hearts:
            SURF.blit(
        ...
        for i in range(empty_hearts):
            SURF.blit(
                ...
                (
                    HEART_X_START + (full_hearts + half_hearts + i) * HEART_SPACING,
                    HEART_Y,
                ),
            )
...
class World:
    ...
    def draw(this):
        for y in range(
            player.camera.top // BLOCK_SIZE, (player.camera.bottom // BLOCK_SIZE) + 1
        ):
            for x in range(
